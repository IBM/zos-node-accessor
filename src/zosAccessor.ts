...
// Input for uploading
type Input = NodeJS.ReadableStream | Buffer | string;
...
class ZosAccessor {
   ...
   * @returns Promise to return void
   */
  public async allocateDataset(
    datasetName: string,
    allocateParamsOrString?: string | AllocateParams
  ): Promise<void> {
    const ftpClient = this.client;
  ...
  private async download(
    ...
    ftpClient[transferMode]((err: Error) => {
      ...
      ftpClient.site(
        "FILETYPE=SEQ TRAILINGBLANKS " + sbsendeol,
        (err1: Error) => {
          if (err1) {
          ...
          ftpClient.get(dsn, (err2: Error, readStream: ReadStream) => {
            ...
            readStream.on("data", (chunk: Buffer) => {
              if (bytes + chunk.length >= this.maxBufferSize) {
                const message =
                  `Reach the maximum buffer size, ${this.maxBufferSize} bytes. ` +
      ...
      ftpClient.list(dsn, (err1: FtpError, list: string[]) => {
        if (err1) {
          if (
            err1.code === 550 &&
            list === undefined &&
            /No [\s\w]+ found/.test(err1.message)
          ) {
  ...
  public async deleteFile(
    filePath: string,
    fileType: FileToOperate = FileToOperate.FILE_OR_DIRECTORY
  ): Promise<void> {
    const deferred = Q.defer<void>();
    const ftpClient = this.client;
        ...
        for (const item of list) {
          ...
          if (entry.fileType === FileType.DIRECTORY) {
            await this.deleteFile(
              ...
              FileToOperate.WHOLE_DIRECTORY
            );
          } else {
            await this.deleteFile(
              path.join(filePath, entry.name),
              FileToOperate.FILE_OR_DIRECTORY
            );
          }
        }
    ...
    return this.listJobs(option).then((jobs: Job[]) => {
      for (const job of jobs) {
        if (
          job.jobId &&
          queryOption.jobId &&
          job.jobId.toUpperCase() === queryOption.jobId.toUpperCase()
        ) {
          if (job.status === "INPUT") {
   ...
   */
  public async getRCFromJESMSGLG(
    queryOption: JobLogOption
    ...
    return this.getJobLog(queryOption).then((log: string | SpoolFile[]) => {
      ...
      if (logContents) {
        ...
        logContents.split("\n").forEach((line: string) => {
          ...
          if (posn1 !== -1) {
            rc = parseInt(
              line.substring(posn1 + EYE_CATCHER_1.length).trim(),
              10
            );
          } else if (posn2 !== -1) {
    ...
    ftpClient.ascii((err: Error) => {
      ...
      ftpClient.site(
        ...
        (err1: Error) => {
          ...
          ftpClient.list(
            option.jobId as string,
            (err2: Error, list: string[]) => {
              ...
              if (spoolFileIndex !== -1) {
                ...
                jobStatus.spoolFiles = parseSpoolTable(
                  list.slice(spoolFileIndex),
                  this.migrationMode
                );
              }
              ...
              if (jobStatus.status === "OUTPUT" && jobStatus.rc === undefined) {
                ...
                if (spoolFiles !== undefined) {
                  ...
                  if (file) {
                    ...
                    this.getRCFromJESMSGLG(optionForJESMSGLG).then(
                      (rc: number | string) => {
                        jobStatus.rc = rc;
