 ...
 *  ]}
 */
ZosAccessor.prototype.getJobStatus = function (jobId, jobName) {
    jobName = jobName || '*';  // Optional jobName
    var deferred = Q.defer();
    var ftpClient = this.client;
    ftpClient.ascii(function (err) {
        if (err) {
 ...
 * @param jobId
 * @param spoolFileIndex - Index of the spool file to get.
 *          Number of spool files can be found using `getJobStatus`,
 *          specifying 'x' will return all spool files joined with the `!! END OF JES SPOOL FILE !!`
 */
ZosAccessor.prototype.getJobLog = function (jobName, jobId, spoolFileIndex) {
    var deferred = Q.defer();
    var ftpClient = this.client;
    jobName = jobName || '*';
    if (spoolFileIndex === undefined) {
        spoolFileIndex = 'x';
    }
    ftpClient.ascii(function (err) {
        if (err) {
        ...
        ftpClient.site('FILETYPE=JES', function (err, responseText, responseCode) {
            ...
            ftpClient.site('JESJOBNAME=' + jobName, function (err, responseText, responseCode) {
                ...
                ftpClient.site('SBSENDEOL=CRLF', function (err, responseText, responseCode) {
                    ...
                    ftpClient.get(jobId + '.' + (spoolFileIndex === 'metaInfo' ? 'x' : spoolFileIndex), function (err, readStream) {
                        if (err) {
                            return deferred.reject(err);
                        }
                        var chunks = [];
                        ...
                        readStream.on('end', function () {
                            ...
                            if (spoolFileIndex === 'metaInfo') {  // Deprecated
