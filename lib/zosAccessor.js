...
ZosAccessor.prototype.listJobs = function (condition) {
    ...
    ftpClient.ascii(function (err) {
        ...
        var siteParams = ['FILETYPE=JES'],
            supportedQueries = ['JESJOBNAME', 'JESOWNER', 'JESSTATUS'];
        for(var key in condition) {
            if (!condition.hasOwnProperty(key)) {
                continue;
            }
            var jesQueryKey = 'JES' + key.toUpperCase();
            if (supportedQueries.indexOf(jesQueryKey) !== -1) {
                siteParams.push(jesQueryKey + '=' + condition[key]);
            }
        }

        // TODO: restore to previous ftp state, or user should in charge of this?
        callbackPipeline(ftpClient.site.bind(ftpClient), siteParams, function (err) {
            if (err) {
                return deferred.reject(err);
            }
...
/**
 * Execute following pattern in sync mode
 *
 * for-each arg in args:
 *   func(arg)
 *
 */
function callbackPipeline(func, args, done) {
    if (args.length === 0) {
        return done();
    }
    func(args[0], function (err) {
        if (err) {
            done(err);
        } else {
            callbackPipeline(func, args.slice(1), done);
        }
    });
}
