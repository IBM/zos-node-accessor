 ...
 * @param {object} allocateOption - An object whose keys are parameters used when allocating dataset, can be 
 *                                  "LRECL", "RECFM", "BLKSIZE" etc, refer to the doc for more details.
 */
ZosAccessor.prototype.allocateDataset = function(datasetName, allocateOption) {
    return this.uploadDataset(new Buffer(''), datasetName, undefined, allocateOption);
}
 ...
 *                  "LRECL", "RECFM", "BLKSIZE" etc, refer to the doc for more details.
 */
ZosAccessor.prototype.uploadDataset = function(input, destDataset, dataType, allocateOption) {
    var deferred = Q.defer();
    ...
    for(var key in allocateOption || {}) {
        if (!allocateOption.hasOwnProperty(key)) {
            continue;
        }
        if (allocateOption[key] === true) {
            siteParams.push(key.toUpperCase());
        } else {
            siteParams.push(key.toUpperCase() + '=' + allocateOption[key]);
        }
    }
...
ZosAccessor.prototype.getDataset = function (dsn, dataType, stream) {
    ...
    ftpClient[dataType](function (err) {
        ...
        ftpClient.site('FILETYPE=SEQ TRAILINGBLANKS=TRUE ' + sbsendeol, function (err, responseText, responseCode) {
            if (err) {
                return deferred.reject(err);
            }
 ...
 * List a dataset
 *
 * @param {string} dsn - Specify a full qualified dataset name, it can contain a wildcard (* or ?)
 */
