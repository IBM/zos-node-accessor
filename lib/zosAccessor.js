...
ZosAccessor.prototype.allocateDataset = function (datasetName, allocateParams) {
  ...
  return self.listDataset(datasetName).then(function (datasetList) {
    ...
    if (
      allocateParamString.indexOf("DSORG=PO") !== -1 ||
      ...
      ftpClient.site(
        allocateParamString,
        function (err, responseText, responseCode) {
          if (err) {
 ...
 *
 */
ZosAccessor.prototype.uploadDataset = function (
  input,
  destDataset,
  dataType,
  allocateParams
      ...
      ftpClient.list(dsn, function (err, list) {
        if (err) {
          ...
          if (list[0].match(/^total/)) {
            ...
            list[0].indexOf("Volume") >= 0 &&
            list[0].indexOf("Dsname") >= 0
          ) {
            deferred.resolve(self.parseMVSDataSets(list));
          } else if (
            list[0].indexOf("Name") >= 0 &&
            list[0].indexOf("Id") >= 0
          ) {
            deferred.resolve(self.parsePDSMembers(list));
          } else if (
            list[0].indexOf("Name") >= 0 &&
            list[0].indexOf("Amode") >= 0
          ) {
            ...
            deferred.reject(
              new Error("Unrecognized file list header: " + list[0])
            );
          }
        }
      });
...
ZosAccessor.prototype.listJobs = function (jobNameOrOption) {
  ...
  if (typeof jobNameOrOption === "string" || jobNameOrOption === undefined) {
    ...
    option = {
      jobName: jobNameOrOption || "*",
      owner: this.username,
      status: "ALL",
    };
  } else if (typeof jobNameOrOption === "object") {
    if (
        jobNameOrOption.jobName && (jobNameOrOption.jobName).length > 8 ||
        jobNameOrOption.owner && (jobNameOrOption.owner).length > 8
      ) {
        throw new Error("Value of prefix or owner is not valid. It is longer than 8 characters.");
    } else {
    ...
    throw new Error(
      "The first parameter type of listJobs() should be string or object."
    );
  }
...
ZosAccessor.prototype.queryJob = function (jobNameOrOption, jobId) {
  ...
  if (typeof jobNameOrOption === "string" || jobNameOrOption === undefined) {
    ...
    throw new Error(
      "The first parameter type of queryJob() should be string or object."
    );
  }
...
ZosAccessor.prototype.getJobStatus = function (jobIdOrOption) {
  ...
  if (typeof jobIdOrOption === "string") {
    ...
    throw new Error(
      "The first parameter type of getJobStatus() should be string or object."
    );
  }
  ...
  ftpClient.ascii(function (err) {
    ...
    ftpClient.site(
      `FILETYPE=JES SBSENDEOL=CRLF JESJOBNAME=* JESOWNER=${option.owner}`,
      function (err, responseText, responseCode) {
        ftpClient.list(option.jobId, function (err, list) {
          ...
          if (jobBody.length > headers.length) {
            ...
            jobStatus.extra.split(/\s+/).forEach(function (entry) {
              ...
              if (pair.length === 2) {
                ...
                jobStatus["retcode"] =
                  "RC " + (Array(4).join("0") + pair[1]).slice(-4);
              }
            });
          }
          ...
          if (tsoFieldsIndex !== -1) {
            var tsoFieldStr =
              list[tsoFieldsIndex] + " " + list[tsoFieldsIndex + 1];
          ...
          if (jobStatus["extra"] && jobStatus["extra"].includes("error")) {
            ...
            jobStatus["extra"] &&
            jobStatus["extra"].includes("ABEND=")
          ) {
            jobStatus["rc"] = jobStatus["extra"];
...
function parseSpoolTable(spoolFileTable) {
  ...
  for (var i = 0; i < headerLine.length; ++i) {
    if (
      (/\s/.test(lastChar) && !/\s/.test(headerLine[i])) ||
      i === headerLine.length - 1
    ) {
...
ZosAccessor.prototype.getJobLog = function (jobNameOrOption, jobId, fileId) {
  ...
  if (typeof jobNameOrOption === "string" || jobNameOrOption === undefined) {
    ...
    throw new Error(
      "The first parameter type of getJobLog() should be string or object."
    );
  }
  ...
  ftpClient.ascii(function (err) {
    ...
    ftpClient.site(
      `FILETYPE=JES SBSENDEOL=CRLF JESJOBNAME=${option.jobName} JESOWNER=${option.owner}`,
      function (err, responseText, responseCode) {
        ftpClient.get(
          option.jobId + "." + option.fileId,
                ...
                ftpClient.list(jobId, function (err, list) {
                  ...
                  if (headerIndex === -1) {
                    return deferred.reject(
                      new Error("Cannot find job header line")
                    );
                  }
                  var spoolFiles = parseSpoolTable(list.slice(headerIndex));
                  logString
                    .split(/\s*!! END OF JES SPOOL FILE !!\s*/)
                    .filter(function (l) {
                      return l;
                    })
...
ZosAccessor.prototype.deleteJob = function (jobIdOrOption) {
  ...
  if (typeof jobIdOrOption === "string") {
    ...
    throw new Error(
      "The first parameter type of deleteJob() should be string or object."
    );
  }
    ...
    ftpClient.site("FILETYPE=JES", function (err, responseText, responseCode) {
      ...
      ftpClient.put(Buffer.from(JCLText), "PLACEHOL", function (err) {
        if (err) {
          ...
          var matched =
            arguments.callee.caller.arguments[1].match(/JES as (\w+)\s/i);
          if (!matched || matched.length !== 2) {
            deferred.reject(
              new Error("Failed to submit jcl, job id not found")
            );
          } else {
