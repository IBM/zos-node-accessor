...
/**
 * Get JCL RC by reading JESMSGLG. It's written for queryJob(), and its parameter is as same as
 * queryJob().
 */
ZosAccessor.prototype.getRCFromJESMSGLG = function (option) {
    let self = this;
    return this.getJobStatus(option).then(function(jobStatus) {
        if (jobStatus && jobStatus.spoolFiles) {
            let file = jobStatus.spoolFiles.find(function(spoolFile) {
        ...
        if (file) {
            ...
            return self.getJobLog(jobLogOption);
        }
        return undefined;
    }).then(function(log) {
        let rc = undefined;
        if (log) {
            ...
            log.split('\n').forEach(function(line) {
                let posn = line.indexOf(EYE_CATCHER);
                if (posn !== -1) {
