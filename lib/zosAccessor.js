...
/**
 * Connect to z/OS via FTP
 * 
 * username - Username for authentication on z/OS.
 ...
 * pasvTimeout - How long (in milliseconds) to wait for a PASV data connection to be established, default is 30000.
 */
ZosAccessor.prototype.connect = function (username, password, host, port, connTimeout, pasvTimeout) {
    ...
    this.client.connect({
        ...
        connTimeout: connTimeout || 10000,
        pasvTimeout: pasvTimeout || 30000 // 30 seconds
    });
    ...
    ftpClient.ascii(function (err) {
        ...
        ftpClient.site('FILETYPE=SEQ', function (err, responseText, responseCode) {
            ...
            ftpClient.list(dsn, function (err, list) {
                if (err) {
                    if (err.code === 550 && list === undefined) {
                        // No dataset is found.
                        deferred.resolve([]);
                    } else {
                        deferred.reject(err);
