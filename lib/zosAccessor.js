...
/**
 * Get jes spool files identified by jobName and jobId, each spool file includes its ddname, stepname, procstep, file content etc.
 * 
 * jobName - Name of the job
 * jobId - ID of the job
 */
                ...
                ftpClient.site('SBSENDEOL=CRLF', function (err, responseText, responseCode) {
                    ...
                    ftpClient.get(jobId + '.x', function (err, readStream) {
                        ...
                        readStream.on('end', function () {
                            ...
                            if (logFormat !== 'withMetaInfo') {
                                return deferred.resolve(logString);
                            }
                            var spoolFileContents = logString.split(/\s*!! END OF JES SPOOL FILE !!\s*/)
                                    .filter(function(l){ return l;}),
                                // Pre-fill with file content, in case of any error when fetching the meta info
                                spoolFiles = spoolFileContents.map(function (c) {return {content: c};});
                            ...
                            ftpClient.list(jobId, function (err, list) {
                                ...
                                **/
                                if (err) {
                                    debug('Failed to list by job id: ' + jobId,  err);
                                } else {
                                    var headerIndex = list.findIndex(function (line) {
                                        return /^\s+ID\s+/i.test(line);
                                    });
                                    if (headerIndex === -1) {
                                        debug('Cannot find job header line');
                                    } else {
                                        var headers = list[headerIndex].toLowerCase().split(/\s+/);
                                        var spoolFileDescIndex = headerIndex + 1;
                                        while (list[spoolFileDescIndex].split(/\s+/).length === headers.length) {
                                            var columns = list[spoolFileDescIndex].split(/\s+/);
                                            // The spool file I'm going to add more info
                                            var spool = spoolFiles[spoolFileDescIndex - headerIndex - 1] || {};
